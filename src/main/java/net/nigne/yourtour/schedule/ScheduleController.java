...
public class ScheduleController {
   ...
   public ModelAndView scheduleState(HttpServletRequest request, ScheduleModel scheduleModel, ScheduleDayModel scheduleDayModel, HttpSession session) throws Exception{
      ...
      System.out.println("sch_no=" + scheduleModel.getNo());
   ...
   public ModelAndView scheduleCommentWrite(HttpServletRequest request,HttpSession session, ScheduleModel scheduleModel ) throws Exception{
	   ...
	   scheduleModel = scheduleService.scheduleSelectOne(scheduleModel);
	   session.getAttribute("session_m_email");
	   scheduleCommentModel.setEmail((String) session.getAttribute("session_m_email"));// 쓴사람 이메일
	   scheduleCommentModel.setSch_no((scheduleModel.getNo())); //글넘버
