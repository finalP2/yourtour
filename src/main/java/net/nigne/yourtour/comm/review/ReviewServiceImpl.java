	...
	public void updateBoard(Map<String, Object> map, HttpServletRequest request) throws Exception{
		...
		if(tempFilenames.length > 1) {  // 잘리는게 있으면 길이가 2 이상이 나온다. 
			...
			for(int i=0; i<tempFilenames.length; i++) {
				tempFilenames[i] = tempFilenames[i].substring(0, tempFilenames[i].indexOf("\""));
				filenames[i] = tempFilenames[i].substring(tempFilenames[i].indexOf("_")+1);
				finalName = fileUtils.moveToReal(tempFilenames[i], "review", articleid, filenames[i]);
			}	
			// 아래: replaceAll(정규식) 이용해서 'GetTempFile' --> 'GetFile'
			// 아래: 파일주소의 time 부분을 제거하고, '리뷰' 게시판정보(review)와 글번호를 대신 넣어준다
			String target = source.replaceAll("GetTempFile.go.filename=[a-zA-Z0-9`~!@#$%^&()-_=+|\\[\\]{};',. ]+\\....\"", "GetFile.go?cate=review&idx="+articleid+"&filename="+finalName+"\"");
			System.out.println(target);
...
//			- 삭제된 태그가 있으면 없애주고..
		...
		if(tags.size() > 0 && !tags.get(0).toString().trim().isEmpty()) {	// isEmpty 안하고 "" 이걸로 비교했더니 안되네;;;;
			...
			// reviewDAO.removeTags(removeMap);
			if(oriTags.size() > 0 && !oriTags.get(0).toString().trim().isEmpty()) {
				...
				for(int i=0; i<oriTags.size(); i++) {
					...
					int tagId = reviewDAO.getTagId(removeMap.get("TAG").toString());
					removeMap.put("TAGID", tagId);
					
					reviewDAO.removeTag(removeMap);
					reviewDAO.removeTagmap(removeMap);			
				}
				map.put("TAG", "");
				reviewDAO.updateArticleTag(map);
			}
		}		
	}
